...
elements = np.zeros(shape=(n_elements,4),dtype = int)
...
with open("E:\\2D_wrinklexml.txt","w") as f:
    ...
    f.write('  <mesh celltype="triangle" dim="2">')  #设置文件对象
    f.write('\n')
    ...
    for i in range(n_nodes):   #对于双层列表中的数据
        nnn = str('      ') + str('<vertex index="') + str(i) + str('" x="') + str(nodes[i,1]) + str('" y="') + str(nodes[i,2]) +str('" />')
        f.write(nnn)
        f.write('\n')
    ...
    f.write('\n')
    for i in range(n_elements):
        nnn = str('      ') + str('<triangle index="') + str(i) + str('" v0="') + str(elements[i,1]) + str('" v1="') + str(elements[i,2]) + str('" v2="') + str(elements[i,3]) + str('" />')                                                                #对于双层列表中的数据
        f.write(nnn)
        f.write('\n')
    f.write('    </cells>')
    f.write('\n')
